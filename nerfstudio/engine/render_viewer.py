...
class Trainer:
       ...
       # self.grad_scaler = GradScaler(enabled=self.mixed_precision)
        ...
        if self.config.is_viewer_enabled() and local_rank == 0:
            self.viewer_state, banner_messages = viewer_utils.setup_viewer(
                config.viewer, log_filename=viewer_log_path, datapath=None #config.pipeline.datamanager.dataparser.data
            )
    ...
    def _load_checkpoint(self) -> None:
        ...
        config = yaml.load(self.config.load_dir.read_text(), Loader=yaml.Loader)
