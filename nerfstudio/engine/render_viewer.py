...
from nerfstudio.configs.experiment_config_render_view import ExperimentConfig
...
from nerfstudio.pipelines.base_pipeline_render_view import VanillaPipeline
from nerfstudio.utils import profiler_render_view, writer_render_view
...
from nerfstudio.utils.writer_render_view import EventName, TimeWriter
from nerfstudio.viewer.server import viewer_utils
from nerfstudio.pipelines.base_pipeline_render_view import Pipeline
...
class Trainer:
    ...
    def __init__(self, config: TrainerConfig, local_rank: int = 0, world_size: int = 1):
        ...
        viewer_log_path = config.viewer.relative_log_filename
        self.viewer_state, banner_messages = None, None
        if self.config.is_viewer_enabled() and local_rank == 0:
            self.viewer_state, banner_messages = viewer_utils.setup_viewer_for_render(
    ...
    @staticmethod
    def eval_load_checkpoint(config: RenderViewerConfig, pipeline: Pipeline) -> Path:
...
#             CONSOLE.print("No checkpoints to load, training from scratch")
    ...
    @profiler_render_view.time_function
    ...
    def eval_iteration(self, step):
        ...
        if step_check(step, self.config.steps_per_eval_batch):
            ...
            writer_render_view.put_scalar(name="Eval Loss", scalar=eval_loss, step=step)
            writer_render_view.put_dict(name="Eval Loss Dict", scalar_dict=eval_loss_dict, step=step)
            writer_render_view.put_dict(name="Eval Metrics Dict", scalar_dict=eval_metrics_dict, step=step)

        # one eval image
        if step_check(step, self.config.steps_per_eval_image):
            with TimeWriter(writer_render_view, EventName.TEST_RAYS_PER_SEC, write=False) as test_t:
            ...
            writer_render_view.put_dict(name="Eval Images Metrics", scalar_dict=metrics_dict, step=step)
            group = "Eval Images"
            for image_name, image in images_dict.items():
                writer_render_view.put_image(name=group + "/" + image_name, image=image, step=step)
