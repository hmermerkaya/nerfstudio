...
@check_main_thread
def setup_viewer_for_render(config: cfg.ViewerConfig, log_filename: Path):
    """Sets up the viewer if enabled

    Args:
        config: the configuration to instantiate viewer
    """
    viewer_state = ViewerStateForRender(config, log_filename=log_filename)
    banner_messages = [f"Viewer at: {viewer_state.viewer_url}"]
    return viewer_state, banner_messages
...
class ViewerStateForRender:
    ...
    def init_scene(self, start_train=False) -> None:
        """Draw some images and the scene aabb in the viewer.
